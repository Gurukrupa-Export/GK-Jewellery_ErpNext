...
class CADOrder(Document):
	...
	def validate(self):
			...
			# 		# # if self.workflow_state in ["Sent to QC","Designing - On-Hold", "Cancel"]:
			# 		# if self.workflow_state == "Sent to QC":
			# 		# 	time_log.to_time = now_datetime()
			# 		# else:
			# 		# 	time_log.to_time = None

			# 		timesheet_doc.save()

			# 	frappe.msgprint("Timesheets created or updated for each designer assignment")
...
def create_line_items(self):
	...
	if self.item_type == 'Template and Variant':
		if self.design_type != 'Sketch Design':
			...
			item_variant = create_variant_of_template_from_cad_order(item_template,self.name)
		...
		if self.design_type != 'Sketch Design':
			item_variant = create_only_variant_from_cad_order(self,self.name)
		...
		if self.design_type != 'Sketch Design':
			item_template = create_sufix_of_variant_template_from_cad_order(self)
			frappe.db.set_value('Item',item_template,'modified_from','')
			updatet_item_template(item_template)
			item_variant = create_variant_of_sufix_of_variant_from_cad_order(self,item_template,self.name)
			update_item_variant(item_variant,item_template)
			frappe.msgprint(_("New Item Created: {0}".format(get_link_to_form("Item",item_variant))))
			frappe.db.set_value(self.doctype, self.name, "item", item_variant)
		else:
...
def create_variant_of_template_from_cad_order(item_template,source_name, target_doc=None):
	def post_process(source, target):
		target.order_form_type = 'CAD Order'
		target.custom_cad_order_id = source_name
		target.custom_cad_order_form_id = frappe.db.get_value('CAD Order',source_name,'cad_order_form')
		...
		for i in frappe.get_all("Attribute Value Item Attribute Detail",{'parent': subcateogy,'in_item_variant':1},'item_attribute',order_by='idx asc'):
			...
			try:
				attribute_value = frappe.db.get_value('CAD Order',source_name,attribute_with)
			except:
				attribute_value = ' '
...
def create_only_variant_from_cad_order(self,source_name, target_doc=None):
...
def create_sufix_of_variant_template_from_cad_order(source_name, target_doc=None):
...
def create_variant_of_sufix_of_variant_from_cad_order(self,item_template,source_name, target_doc=None):
	def post_process(source, target):
		target.order_form_type = 'CAD Order'
		target.custom_cad_order_id = source_name
		target.custom_cad_order_form_id = frappe.db.get_value('CAD Order',source_name,'cad_order_form')
