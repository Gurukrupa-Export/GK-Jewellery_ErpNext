...
frappe.ui.form.on('Demo Order Form', {
...
frappe.ui.form.on('Demo Order Form Details', {
	order_details_add(frm,cdt,cdn) {
		...
		row.idx_ = `${frm.doc.order_details.length}`;
		// refresh_field("order_details");

		var childTable2 = cur_frm.add_child("order_details_2");
		childTable2.idx_=row.idx_
		cur_frm.refresh_fields("order_details_2");
	},
	order_details_remove(frm,cdt,cdn) {
		const order_detail_idx_arr = []
		for (var i = 0; i < frm.doc.order_details.length; i++) {
			order_detail_idx_arr.push(frm.doc.order_details[i].idx_);
		}

		for (var j = 0; j < frm.doc.order_details_2.length; j++) {
			if(order_detail_idx_arr.includes(frm.doc.order_details_2[j].idx_)==false){
				console.log(frm.doc.order_details_2[j].idx_)
				cur_frm.get_field("order_details_2").grid.grid_rows[j].remove();
			}
		}
		cur_frm.refresh();
	},
	...
	subcategory: function (frm, cdt, cdn) {
		var d = locals[cdt][cdn]
		// var childTable2 = cur_frm.add_child("order_details_2");
		// childTable2.subcategory=d.subcategory
		// cur_frm.refresh_fields("order_details_2");
		set_field_visibility(frm, cdt, cdn)
	},
