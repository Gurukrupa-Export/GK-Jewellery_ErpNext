...
# @frappe.whitelist()
# def get_gold_price_for_mrp():
# 	data_list = []
	
# 	url = frappe.db.get_value('Gold Price Scrape Settings',{'name':'For MRP'},'api_url',order_by='creation desc')
...
@frappe.whitelist()
def get_gold_price_for_all_territories():
	url = frappe.db.get_value('Gold Price Scrape Settings',{'name':'For All India'},'api_url',order_by='creation desc')
	session = requests.Session()
	response = session.get(url).json()
	gold_rate = response['gold_price']
	return gold_rate