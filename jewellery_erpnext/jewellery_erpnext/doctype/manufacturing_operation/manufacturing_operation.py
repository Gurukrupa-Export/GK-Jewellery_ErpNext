			...
			for d in self.get("time_logs")[-1:]:
				...
				if d.from_time and d.to_time:
					...
					for i in self.get("time_logs"):
						...
						self.total_time_in_mins += i.time_in_mins

					default_shift = frappe.db.get_value('Employee',d.employee,'custom_shift_type')
					if default_shift==None:
						frappe.throw(f'Default Shift For {d.employee} employee is not available')
